...
A Rust toolchain is required to compile the native shell; the Web UI fallback
remains runnable without one via `deno task ui`. Building and packaging the
shipped macOS artifact is documented in the repository README.
...
and only to an explicitly selected output directory. Secret commands go through
the macOS Keychain adapter (desktop shell) or the local service's Keychain
store (browser shell); no secret material is ever written as plain text.
